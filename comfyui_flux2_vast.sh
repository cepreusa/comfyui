...
provisioning_create_default_workflow() {
    ...
    # Скачиваем рабочий workflow из GitHub
    WORKFLOW_URL="https://raw.githubusercontent.com/cepreusa/comfyui/refs/heads/main/Flux2.json"
    
    if wget -q -O "${COMFYUI_DIR}/user/default/workflows/flux2_default.json" "$WORKFLOW_URL"; then
        echo "✓ Default workflow скачан из GitHub"
    else
        echo "⚠ Не удалось скачать workflow из GitHub"
    fi
